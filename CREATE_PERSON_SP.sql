...
IS
    ...
    BEGIN
        ...
        ELSIF p_person_email IS NULL THEN
            ex_error_msg := 'Missing email addres! Can not be NULL';
            RAISE ex_exception;
        ELSE
            p_person_ID := PERSON_ID_SEQ.NEXTVAL;

            INSERT INTO VM_PERSON VALUES (p_person_ID, p_person_email, p_person_given_name, p_person_surname, p_person_phone);
            COMMIT;

            DBMS_OUTPUT.PUT_LINE('Person added with ID: ' || p_person_ID);
        END IF;

        IF p_person_given_name IS NULL AND p_person_surname IS NULL THEN
            OPEN check_person_id;
                FETCH check_person_id INTO checked_person_id;
                IF check_person_id%FOUND THEN
                    ex_error_msg := 'Email "' || p_person_email || '" is in use by person ID: ' || checked_person_id;
        ...
        EXCEPTION
            WHEN DUP_VAL_ON_INDEX THEN
                ...
                FROM VM_PERSON
                WHERE PERSON_EMAIL = p_person_email;
